...
public class XmlUtils extends DefaultHandler {
	...
	public static XStream jstream = new XStream(new JsonHierarchicalStreamDriver());
	...
	private boolean appendRootNode = false;
	private short nodeLevel = 0;
	
	public XmlUtils () {}
	
	public XmlUtils(boolean appendRootNode)
	{
		this.appendRootNode = appendRootNode;
	}
	
	public HashMap<String, Object[]> toMap( String xmlSnippet) {
		
		if ( StringUtils.isEmpty(xmlSnippet)) return null;
	...
	public String toString(String xmlSnippet) throws IllegalArgumentException {
		...
		for (String key : res.keySet()) {
			...
			sb.append(key).append('=').append(StringUtils.arrayToString(values) );
		}
		return sb.toString();
	}
	...
	{
		this.nodeLevel++;
		if (!this.appendRootNode && this.nodeLevel == 1) return;
		
		if ( null == currentNode ) currentNode =  qName;
		else currentNode = currentNode + "." + qName;
		
		//System.out.println("startElement.currentNode:" + currentNode);
	}
	...
	//	System.out.println("endElement.currentNode:" + currentNode + " : appendedQName = " + appendedQName);
		...
		{
			if (val.length() != 0 ) this.xmlMap.put(currentNode, new String[]{val} );
		}
		
		System.out.println(currentNode + " = " + val);
		if (appendedQName.length() == (currentNode.length() + 1)) currentNode = null;
		else currentNode = currentNode.substring(0, currentNode.length() - appendedQName.length());
		this.sb.delete(0, sb.capacity());
	}
